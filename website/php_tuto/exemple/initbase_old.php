...
if (isset($_POST['bd_root'])) {  // Second passage : création de la base
	...
	if ($erreur == 0) {
		@mysql_connect($bdServeur, $bdRoot, $bdRootPass) or $erreur = -1;
		if ($erreur == 0) {
			@mysql_select_db($bdNom) or $erreur = 1;
		}
		// si $erreur est encore égale à 0, cela veut dire que la base existe déjà
		if ($erreur == 0) {
			$erreur = -3;
		}
	}
	...
	if ($erreur == 1) {
		$erreur = fp_make_db($bdNom, $bdNewUser, $bdNewPass);
	}

	switch ($erreur) {
		case -1:
			echo '<p class="err">Impossible de se connecter au serveur. Erreur ',mysql_errno().' : '.mysql_error(), '</p>';
			break;
		case -2:
			echo '<p lass="err">Vous devez saisir des informations valides</p>';
			break;
		case -3:
...
echo '<p>Vous allez créer et initialiser la base de données ',
	...
	'<input type="text" name="bd_root_pass" size="20" value="', $bdRootPass, '"></label>',
 ...
 */
function fp_make_db($bdNom, $bdNewUser, $bdNewPass) {
	// Création de la base
	$R = @mysql_query("CREATE DATABASE $bdNom");
	if (!$R) {
		exit('Erreur initBase - '.__LINE__.' : création de la base de données impossible<br>'
				.mysql_errno().' : '.mysql_error());
	}

	@mysql_select_db($bdNom);
	...
	foreach ($creates as $table => $S) {
		...
		if (@mysql_query($inserts[$table]) === FALSE) {
			echo 'Erreur initBase - ', __LINE__,
	...
	$S = 'GRANT SELECT, INSERT, UPDATE, DELETE ON '.$bdNom.'.* TO "'.$bdNewUser.'"@"localhost" IDENTIFIED BY "'.$bdNewPass.'"';
	if (@mysql_query($S) === FALSE) {
		echo 'Erreur initBase - ', __LINE__,
